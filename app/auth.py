...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    API requests (under /api/) that are not authenticated receive 401.
    Page requests are redirected to /login.
    Static files and the login/logout routes are always public.
    """

    async def dispatch(self, request: Request, call_next):
        path = request.url.path

        # Allow bypassing auth in test/dev environment
        if _AUTH_DISABLED:
